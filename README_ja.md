...
‡ 冗長 IMU を備えた**オートパイロット級**ボード: AF-F7 mini は PWM 出力を直接駆動（IO コプロセッサなし）し、AF-H7E は STM32F103 IO コプロセッサとイーサネットを搭載したモジュラー設計です。ボード ID `6201` / `6202`（novaX-ALUX 予約レンジ `6200`–`6209`）。
